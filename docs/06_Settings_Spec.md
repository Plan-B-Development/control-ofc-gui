...
- **`AppSettings.from_dict` is the trust boundary (DEC-137):** it never raises.
  ...
  spin-down 5–12 s on every machine — DEC-426 removed DEC-329's lower OpenFan ceiling,
  because the daemon does not restart a stopped OpenFan fan), `card_size` is an enum, `window_geometry`

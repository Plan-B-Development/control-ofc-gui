...
- **Recovery**: Apply a 60% PWM recovery floor for two cycles (the release cycle and one more) to the fans the profile controls, then resume active profile control. Every other fan the emergency took is given back (DEC-382)
- **Fallback**: Apply a 40% PWM floor to the fans the active profile controls if no CPU sensor is reachable for 5 consecutive poll cycles. Fans no profile controls stay under their firmware curve, and with no profile active nothing is forced (DEC-382)

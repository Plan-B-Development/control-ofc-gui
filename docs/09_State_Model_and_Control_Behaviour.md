...
The daemon owns the thermal ladder: at the trip point it forces every OpenFan channel and writable hwmon header the machine has to 100 %, holds until 80 °C, recovers at a 60 % floor, and applies a 40 % floor if no CPU sensor is found for 5 cycles (GPU fans are excluded by design — DEC-130).

...
|---|---|---|---|
| `nzxt-kraken3` (`x53`, `z53`, `kraken2023`, `kraken2023elite`; `kraken2024elite` from kernel 7.3) | NZXT Kraken X/Z-series, 2023, 2023 Elite, 2024 Elite | yes | **writable** — `pwm1` pump (+ `pwm2` fan on Z/2023/2024). The driver labels channel 1 "Pump speed", which the daemon's pump rule matches on every model. The daemon's cooler list does not yet include `kraken2024elite` (register row BRD-c), so that model is not flagged as an AIO and its radiator fan gets the chassis floor rather than the cooler one — its pump is still protected by the label |
...
| `arctic_fan_controller` (`arctic_fan`) | ARCTIC Fan Controller (10 channels), kernel 7.2+ | no | **writable, with a hazard.** There is no `pwm_enable`, and every command the driver sends carries all ten channels. On 7.2 and 7.3 its duty cache starts at 0 at probe and after resume, so **the first write to one channel sends 0% to the other nine** until each has been written. A profile that controls only some of its channels stops the rest. Control all ten channels or none (register row BRD-d); a change queued for kernel 7.4 starts the cache at 40% |

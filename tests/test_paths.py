...
def test_config_dir_ends_with_control_ofc():
    assert config_dir().name == "control_ofc"
...
def test_state_dir_ends_with_control_ofc():
    assert state_dir().name == "control_ofc"
...
def test_ensure_dirs_creates_directories(tmp_path, monkeypatch):
    ...
    ensure_dirs()
    assert (tmp_path / "config" / "control_ofc" / "profiles").is_dir()
    assert (tmp_path / "config" / "control_ofc" / "themes").is_dir()
    assert (tmp_path / "state" / "control_ofc").is_dir()
    assert (tmp_path / "cache" / "control_ofc").is_dir()

...
**Latest release:** v2.58.0 — 2026-09-05. Pairs with `control-ofc-daemon` ≥ v2.11.0. Individual features gate on newer daemons and say so in the app when one is missing; [CHANGELOG.md](CHANGELOG.md) records which version introduced each.
...
Daemon configuration: see the [daemon repo](https://github.com/Plan-B-Development/control-ofc-daemon) and the [Operations Guide](https://github.com/Plan-B-Development/control-ofc-gui/blob/main/docs/18_Operations_Guide.md).

## Architecture

The GUI is a **daemon API client only**. All hardware access goes through the daemon's HTTP-over-Unix-socket API.

As of **2.0.0** the daemon owns all runtime control — it evaluates curves and is the sole writer of every fan backend. The GUI authors profiles, polls, and renders; it never writes PWM.
...
See the [architecture docs](https://github.com/Plan-B-Development/control-ofc-gui/blob/main/docs/02_System_Architecture_and_Boundaries.md) and [API contract](https://github.com/Plan-B-Development/control-ofc-gui/blob/main/docs/08_API_Integration_Contract.md) for details.

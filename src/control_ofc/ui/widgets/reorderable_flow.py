"""Drag-to-reorder flow base (DEC-187).

Shared drag-reorder behaviour for widgets that arrange fixed-size cards in a
:class:`~control_ofc.ui.widgets.flow_layout.FlowLayout`: press-and-drag a card, a
...
:meth:`_attach_drag` when the card is added.
"""
...
class ReorderableFlow(QWidget):
    """Base for a flow container whose cards can be drag-reordered.

    Subclasses must:

    - create a :class:`FlowLayout` and return it from :meth:`flow_layout`;
    - tag each draggable card via :meth:`_attach_drag` (sets its ``card_id``
      property and installs the reorder event filter) when the card is added.

    The base emits :attr:`order_changed` with the new ``list[str]`` of card ids
    ...
    def flow_layout(self) -> FlowLayout:
        """Return the live FlowLayout (subclass-owned)."""
        raise NotImplementedError

    def _attach_drag(self, card: QWidget, card_id: str) -> None:
        """Tag a card with its id and install the reorder event filter.

        Call when a card is added to the layout; the card's own interactive
        children keep their events (the filter is installed on the card only, so
        clicking a child does not start a drag)."""
    ...
    def dragMoveEvent(self, event) -> None:
        if not event.mimeData().hasText():
            return
        drop_index = self._index_at_position(event.position().toPoint())
    ...
    def dropEvent(self, event) -> None:
        ...
        if drop_index > source_index:
            drop_index -= 1
        layout.insertWidget(drop_index, source_widget)
        layout.invalidate()
        self.updateGeometry()

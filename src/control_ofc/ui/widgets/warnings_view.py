"""Active-warnings view (DEC-222) — the Logs page's warnings surface.

Renders ``AppState.active_warnings``: the dedup-keyed, dismissable set (stale
sensor/fan, fan stall, API skew, …). This is **not** the diagnostics event log —
the Logs page shows both, side by side: the event feed is history, this is what
is wrong *right now*.

Extracted from ``dashboard_inspector`` when the Dashboard rebuild removed the
status strip that used to open it in a dialog (DEC-222). The Logs page is now the
single warnings surface, so the view and its per-row "next action" guidance moved
to a module of their own rather than staying in a Dashboard widget file.
"""

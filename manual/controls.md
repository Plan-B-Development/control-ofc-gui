...
|--------------|---------|
| **Title row** | Role name plus a status chip: **Applied** (curve output being written), **Not writing** (a hardware diagnostic — a verify, PWM characterisation, OpenFan calibration or validation sweep — has the daemon's write pause, so the daemon is still working out this control's speed but is not applying it, and the fans hold their last duty), **Manual** (your own inline override active), **External** (an override held by something else — another client, or this app before it restarted), **Not controlled** (the daemon cannot work out a speed for this role, so nothing is driving these fans and they hold their last speed — the chip carries how long it has been that way, and the tooltip the reason), or **No members** |

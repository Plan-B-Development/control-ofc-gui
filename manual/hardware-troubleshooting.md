...
- **`rdna_mes_hang_drm_amd_4765` (Critical):** raised on Linux **6.18.0–6.18.6** and **6.17.9–6.17.13** with an RDNA3, RDNA3.5 or RDNA4 GPU (RX 7000 / RX 9000 series, Radeon Pro W7000 and AI PRO cards, and integrated graphics such as the Radeon 780M, 890M or 8060S). A compute job running alongside a 3D workload can hang the GPU, and while the system is hung nothing can change a fan's speed ([drm/amd #4765](https://gitlab.freedesktop.org/drm/amd/-/issues/4765)). It is **fixed in 6.18.7 and 6.19**. Update to the **latest 6.18 longterm** point release or a current stable **7.x** kernel. **Do not** move to 6.15, 6.16 or 6.17: none of them was ever a longterm kernel, all are end-of-life, and 6.17.9 onward carries this bug. The daemon matches on the version number, so a distribution kernel that already backported the fix may still be flagged.

Daemon v2.56.0 and older raise two older advisories instead. Both were retired because their advice was wrong. Here is what to do if you see one:

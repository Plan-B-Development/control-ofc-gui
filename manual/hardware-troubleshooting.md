...
- **No-sensor fallback (40%):** no CPU sensor has been seen for 5 consecutive poll cycles. The fans your active profile controls are held at 40% or more — so profile fans sitting at a uniform ~40% (rather than 100%) usually mean a missing CPU sensor, not an overheat. Fans no profile controls are left to your motherboard's own fan curve, which reads its own CPU sensor; with no profile active, nothing is forced at all.

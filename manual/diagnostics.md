...
- **Board notes for this hardware (N)** — a collapsed section below the conditions, holding the documented quirks for your board and chip. These are *reference material*, not alarms: they match on which motherboard you own, so they can never clear on their own, and showing them as warnings would train you to ignore the list above. Each note carries a four-tier severity badge (**CRITICAL** red / **HIGH** orange / **MEDIUM**/**LOW** amber / **INFO** blue), a collapsible detail (HIGH and above open by default), and — the part that matters — **what this machine actually says about it**:
  ...
  | *not yet verified on this system* | Neither confirmed nor refuted. Most of these describe the BIOS quietly ignoring PWM writes, which only writing to a header can settle — use **Test fan control** in this section. |
  ...
  **Acknowledge** collapses and greys a note you have read; **Dismiss** hides it. Neither is permanent and neither can silence a real problem — the silence is recorded against what the machine said at the time, so a note you quietened comes back if it is later confirmed. Undo either from **Settings → Prompts & Dismissals**, and turn the buttons off there entirely if you would rather the notes stayed as they are.

...
- **Board notes for this hardware (N)** — a collapsed section below the conditions, holding the documented quirks for your board and chip. These are *reference material*, not alarms: they match on which motherboard you own, so they can never clear on their own, and showing them as warnings would train you to ignore the list above. Each note carries a four-tier severity badge (**CRITICAL** red / **HIGH** orange / **MEDIUM**/**LOW** amber / **INFO** blue), a collapsible detail (HIGH and above open by default), and — the part that matters — **what this machine actually says about it**:
  ...
  | *not yet verified on this system* | Neither confirmed nor refuted. Most of these describe the BIOS quietly ignoring PWM writes, which only writing to a header can settle — use **Test fan control** in this section. A header with no tachometer — an empty header, a 3-pin fan, a fan or pump sitting at zero RPM — reports *no tach* and settles nothing either way; it does not count as a failure, and it will not overwrite a result you already have. A test whose read-back of the header failed outright reports *no readback* (daemon v2.48.0+) and is treated identically — also not a failure, also non-overwriting; re-run it. |
  ...
  **Acknowledge** collapses and greys a note you have read, for this session only; **Dismiss** hides it until you restore it. Neither is permanent and neither can silence a real problem — the silence is recorded against what the machine said at the time, so a note you quietened comes back if it is later **confirmed**. It does *not* come back when things improve: dismiss a note, run **Test fan control**, and a clean result leaves it dismissed. Undo a dismissal from **Settings → Prompts & Dismissals**, and turn the buttons off there entirely if you would rather the notes stayed as they are. Since v2.73.0 the same two buttons appear on every warning this page raises — the conditions, the Interference Monitor, the CPU thermal row, the GPU advisories and, since v2.77.0, the GPU rows on Safety & GPU Limits — and they follow the same rule everywhere. A reading that is *not* warning you, such as **Zero-RPM: available**, has no buttons, because there is nothing to quieten, and the **Hardware Registry** table below has none either — it is a list of what is installed rather than a list of what needs doing.
...
- **Interference Monitor** and **Safety & GPU Limits** — always-visible cards stacked in the column beside the Hardware Registry. The Interference Monitor carries the BIOS-reclaim gauge and names the contended header when the BIOS has been reclaiming fan control. Since v2.74.0 it also says **when**: the daemon's watchdog re-enables manual mode on every reclaim, so a BIOS that is genuinely fighting produces them continuously. Once an hour has passed with none, the warning above stands down and this card reads **Past Interference** — the count is kept, because it never resets while the daemon is running, but it is a record of what happened rather than something to act on. If your daemon is too old to report the timing, nothing stands down and the warning stays exactly as it was; Safety & GPU Limits carries the CPU thermal state — live, from the same 1 Hz daemon poll that drives the banner and the footer, and **Unknown — disconnected** while that poll cannot reach the daemon rather than the last state it saw — with the emergency limit your CPU actually reports beside it, plus the GPU rows and the firmware speed range. Neither is hidden behind a collapse — the page keeps its safety-relevant readings on screen. Both can be quietened, and quietening them **demotes rather than deletes**: the reclaim gauge keeps its count and the thermal row keeps its state and your CPU's own emergency limit, they simply stop being coloured as alarms. Since v2.77.0 the **GPU rows quieten the same way** — **Fan Control: read_only** (the normal state of an RX 7000/9000 card booted without `amdgpu.ppfeaturemask`), **Overdrive: disabled**, either **ppfeaturemask** row, **amdgpu binding: not bound** and any unbound **AMD** PCI device. They are settings you may have chosen on purpose, and they never clear on their own, so leaving them shouting forever was the one part of this page that still did. Dismissing a GPU row does **not** dismiss the matching condition card above it: that card carries the fix, and hiding the instructions with the symptom would be the wrong trade. Change the setting and the row speaks again with its new reading. A real thermal emergency still reaches you through the banner, the footer and the status ribbon, which nothing on this page can silence — and the daemon protects the hardware regardless of what any of them shows.
- **Advanced actions** (the page's one collapsible section, collapsed by default; **Test PWM Control**, **Verify All Writable** and **Characterise** are unavailable while a PWM Test Report is running, because the report is using the daemon's diagnostic slot. They are also unavailable while any one of them is running a test, until it finishes, because the daemon runs these tests one at a time): the **Test PWM Control** / **Verify All Writable** buttons, **Characterise PWM Response** (a deeper PWM/RPM sweep beside the quick test — daemon 2.29.0+, hidden otherwise; see [Hardware Troubleshooting](hardware-troubleshooting.md#characterise-pwm-response)), plus **Test GPU Fan Control** and **Restore GPU Fan to Automatic** when a writable AMD GPU is present.
...
- **PWM Test Report…** — the whole-machine assessment; see [§ PWM Test Report](#pwm-test-report) below. While a report is running, every other test on this page (and on System State) is unavailable, because it is using the daemon's diagnostic slot.
...
**Hardware ▸ PWM Test Report…** (v2.81.0) checks your cooling setup and writes down what it actually saw. It is built on the same daemon tests as the buttons above, run one at a time, and the daemon performs and undoes every one of them.

1. **What to test.** Every channel the daemon reports is listed. For each motherboard header you tick the tests to run: the **PWM control test** and **tach pairing** start ticked on headers with a fan turning; the **full PWM sweep** (100 % down to 20 % and back, never below a pump's floor) and the **stall/restart probe** are yours to choose. The probe (daemon 2.54.0+) looks for where a chassis or radiator fan stops below 20 % and where it starts again; it is never offered on a pump or CPU fan, or on a header without a chassis-fan or radiator-fan role. OpenFan channels and GPU fans are reported but not tested. A test that cannot run says why in its tooltip — including, on a daemon older than 2.52.0, the sweep and tach pairing, whose settling figures were wrong before that version.
2. **Your setup (optional).** Things software cannot see: what is plugged into each header, how many fans share it (a splitter means the RPM is one fan's), the BIOS header mode, and your cooler's model and pump switch position. Anything you leave blank is recorded as *not supplied*. Your answers are remembered for this machine and never included in a settings export.
3. **Review and consent.** The plan in plain words, what keeps it safe, and your go-ahead. Each stall probe also needs its own **"I'll stay at the machine"** tick, because the fan may stop for a short time.
4. **Run.** Each test with its status, the fan under test live, the thermal state and time remaining. **Cancel run** stops the current test (the daemon puts the header back) and lists the rest as not tested. The run stops by itself if the daemon's thermal protection becomes active, and the tests left are listed as not tested. A single test also stops if a temperature passes the daemon's diagnostic limit or temperature readings go stale; the run carries on, but the daemon refuses each later test while that lasts, and those are listed as not tested. Closing the window asks first; quitting Control-OFC saves what was done so far.

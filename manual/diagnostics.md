...
- **Advanced actions** (the page's one collapsible section, collapsed by default; **Test PWM Control**, **Verify All Writable** and **Characterise** are unavailable while a PWM Test Report is running, because the report is using the daemon's diagnostic slot): the **Test PWM Control** / **Verify All Writable** buttons, **Characterise PWM Response** (a deeper PWM/RPM sweep beside the quick test — daemon 2.29.0+, hidden otherwise; see [Hardware Troubleshooting](hardware-troubleshooting.md#characterise-pwm-response)), plus **Test GPU Fan Control** and **Restore GPU Fan to Automatic** when a writable AMD GPU is present.

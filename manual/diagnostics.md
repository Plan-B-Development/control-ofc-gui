...
**A session runs until you stop it.** This is the single most important thing to know about all three recorders: the diagnostics you tick are *short* — seconds to a couple of minutes each — but finishing them does **not** end the session, and neither does closing the window. It keeps recording until you press **Stop & Save**, or until the daemon's sample cap is reached about two hours in. While one is recording, the footer at the bottom of every page shows a **Recording** chip with the sample count; click it to get back to the page where you can stop it.
